...
public class ShadowBoltII : ModNPC
{
    ...
    public override void AI()
    {
        ...
        if (NPC.ai[0] == 0)
        {
            NPC.velocity = Main.rand.NextVector2Circular(12, 12);
            if (Main.getGoodWorld && NPC.ai[1] != 6) NPC.NewNPCDirect(NPC.GetSource_FromThis(),
                                                                      NPC.Center,
                                                                      NPC.type,
                                                                      ai1: 6);
        }
        if (NPC.ai[0] < 30) NPC.velocity /= 1.025f;
        else NPC.velocity *= 1.025f;

        if (NPC.ai[0] == 30)
        {
            vel = NPC.Center.DirectionTo(player.Center) * 16;
        }
        if (NPC.ai[0] >= 30) NPC.velocity = vel;

        if (NPC.ai[0] > 250) NPC.life--;
